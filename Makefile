...
test: train
	...
	@echo "Creating test input file from training data…"
	@python -c "import pandas as pd; df=pd.read_csv('data/training_data.csv', encoding='utf-8'); df[['Job Title']].rename(columns={'Job Title': 'job title'}).reset_index(names='record id').to_csv('data/input.csv', index=False, encoding='utf-8')"
	python scripts/predict.py
...
validate:
	...
	@python -c "import os; import pandas as pd; files = [('data/training_data.csv', 'Training data'), ('data/input.csv', 'Input data'), ('data/keyword_matching.csv', 'Keyword rules'), ('data/title_reference.csv', 'Title mappings')]; [print(f'{name}: {len(pd.read_csv(filepath, encoding=\"utf-8\"))} rows, columns: {list(pd.read_csv(filepath, encoding=\"utf-8\").columns)}') if os.path.exists(filepath) else print(f'{name}: File not found') for filepath, name in files]"
